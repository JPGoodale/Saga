...
Ctx :: struct {
    ...
    kernel_return:          OpReturn,
    kernel_function_end:    OpFunctionEnd,
...
create_storage_buffer :: proc(ctx: ^Ctx, node: Array_Type, element_type_name: string) -> (storage_buffer_id: Id) {
...
create_workgroup_buffer :: proc(ctx: ^Ctx, node: Array_Type, element_type_name: string) -> (workgroup_buffer_id: Id) {
...
translate_block :: proc(ctx: ^Ctx, nodes: [dynamic]Expression) {
    for node, idx in nodes {
...
translate_variable_declaration :: proc(ctx: ^Ctx, node: Variable_Declaration) {
    switch t in node.type {
    case Array_Type:
        element_type_name   := translate_scalar_type(ctx, t.element_type)
...
translate_value_assignment :: proc(ctx: ^Ctx, root_node: Variable_Expression, node: Expression) {
    #partial switch n in node {
    case Literal:
    ...
    case Identifier:
        ...
        load                            : OpLoad
        load.result                     = create_result_id({"%another_", n.name, "_register"})
...
translate_loop_expression :: proc(ctx: ^Ctx, node: Loop_Expression) {
    ...
    start_init: Id
    switch n in node.start {
    case Identifier:
        log.errorf("Not yet!")
    case Literal:
       start_init = create_constant(ctx, n.type, n.value)
    }

    start := create_local_variable(ctx, "i", auto_cast(pointer.result), .Function, start_init)

    end: Id
    switch n in node.end {
    case Identifier:
        log.errorf("Not yet!")
    case Literal:
       end = create_constant(ctx, n.type, n.value)
    }
    ...
    next_i_load.pointer                 = start
    append(&ctx.operations, next_i_load)

    one := create_constant(ctx, "uint32", "1")
    ...
    incrementer.operand_2               = one
    append(&ctx.operations, incrementer)

    i_store                             : OpStore
    i_store.pointer                     = start
...
translate_binary_expression :: proc(ctx: ^Ctx, root_node: Expression, op_node: Binary_Expression, is_subexpr: bool = false) -> (result_id: Id, type: string) {
    // -----------------------------------------------------------------------------------
    // Left-Hand-Side
...
translate_binary_op :: proc(ctx: ^Ctx, node: Binary_Expression, type: string, lhs_load_id, rhs_load_id: Id) -> (result: Result_Id) {
    switch type {
    case "%int8", "%int16", "%int32", "%int64":
        switch node.op {
        case "+":
            op: OpIAdd
            op.result = create_result_id({"%sum_of_", string(lhs_load_id[1:]), string(rhs_load_id[1:])})
        ...
        case "-":
            op: OpISub
            op.result = create_result_id({"%difference_of_", string(lhs_load_id[1:]), string(rhs_load_id[1:])})
            op.result_type = auto_cast(type)
            op.operand_1 = lhs_load_id
            op.operand_2 = rhs_load_id
            append(&ctx.operations, op)
        ...
        case "*":
            op: OpIMul
            op.result = create_result_id({"%product_of_", string(lhs_load_id[1:]), string(rhs_load_id[1:])})
            op.result_type = auto_cast(type)
            op.operand_1 = lhs_load_id
            op.operand_2 = rhs_load_id
            append(&ctx.operations, op)
        ...
        case "/":
            op: OpSDiv
            op.result = create_result_id({"%quotient_of_", string(lhs_load_id[1:]), string(rhs_load_id[1:])})
            op.result_type = auto_cast(type)
            op.operand_1 = lhs_load_id
            op.operand_2 = rhs_load_id
            append(&ctx.operations, op)
        ...
        case "%":
            op: OpSMod
            op.result = create_result_id({"%modulo_of", string(lhs_load_id[1:]), string(rhs_load_id[1:])})
            op.result_type = auto_cast(type)
            op.operand_1 = lhs_load_id
            op.operand_2 = rhs_load_id
            append(&ctx.operations, op)
        ...
        case "%%":
            op: OpSRem
            op.result = create_result_id({"%remainder_of", string(lhs_load_id[1:]), string(rhs_load_id[1:])})
            op.result_type = auto_cast(type)
            op.operand_1 = lhs_load_id
            op.operand_2 = rhs_load_id
            append(&ctx.operations, op)
        ...
        case "<":
            op: OpSLessThan
            op.result = create_result_id({"%lt_comp_of", string(lhs_load_id[1:]), string(rhs_load_id[1:])})
        ...
        case "<=":
            op: OpSLessThanEqual
            op.result = create_result_id({"%lt_or_equal_comp_of", string(lhs_load_id[1:]), string(rhs_load_id[1:])})
            op.result_type = auto_cast("%bool")
            op.operand_1 = lhs_load_id
            op.operand_2 = rhs_load_id
            translate_scalar_type(ctx, "bool")
        ...
        case ">":
            op: OpSGreaterThan
            op.result = create_result_id({"%gt_comp_of", string(lhs_load_id[1:]), string(rhs_load_id[1:])})
            op.result_type = auto_cast("%bool")
            op.operand_1 = lhs_load_id
            op.operand_2 = rhs_load_id
            translate_scalar_type(ctx, "bool")
        ...
        case ">=":
            ...
            op.result = create_result_id({"%gt_or_equal_comp_of", string(lhs_load_id[1:]), string(rhs_load_id[1:])})
            op.result_type = auto_cast("%bool")
            op.operand_1 = lhs_load_id
            op.operand_2 = rhs_load_id
            translate_scalar_type(ctx, "bool")
        ...
        case "==":
            op: OpIEqual
            op.result = create_result_id({"%equal_comp_of", string(lhs_load_id[1:]), string(rhs_load_id[1:])})
            op.result_type = auto_cast("%bool")
            op.operand_1 = lhs_load_id
            op.operand_2 = rhs_load_id
            translate_scalar_type(ctx, "bool")
        ...
        case "!=":
            op: OpINotEqual
            op.result = create_result_id({"%not_equal_comp_of", string(lhs_load_id[1:]), string(rhs_load_id[1:])})
            op.result_type = auto_cast("%bool")
            op.operand_1 = lhs_load_id
            op.operand_2 = rhs_load_id
            translate_scalar_type(ctx, "bool")
...
translate_thread_id :: proc(ctx: ^Ctx, thread_id: Thread_Idx) -> (max_thread_id: Id){
