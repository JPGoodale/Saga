...
//
//
// parser_init :: proc(tokens: []Token, graph: ^IR_Graph) -> Parser {
//     return Parser{
//         graph = graph,
//         curr_scope = graph.scopes[0],
//         tokens = tokens,
//         curr_token = tokens[0],
//     }
// }
...
//
//
// add_symbol :: proc(p: ^Parser, lhs: ^Ast_Node, rhs: ^Ast_Node) {
//     p.curr_scope[lhs.token.value.(string)] = rhs.id
// }
//
//
...
//
//
// parse_file :: proc(p: ^Parser) -> (err: Parsing_Error) {
...
//     module_ast      := make_ir_node(p.graph, .Scope)
//
//     for p.curr_token.kind != .EOF {
